...
function ConnectVisual() {
  return (
    <div className="relative h-60 bg-gradient-to-br from-white/[0.04] to-black/20 flex flex-col items-center justify-center gap-6 p-8">
      <div className="absolute inset-x-0 bottom-0 h-10 bg-gradient-to-b from-transparent to-black/40 pointer-events-none" />
      {/* Clean Spotify logo */}
      <svg
      ...
      {/* Request text */}
      <p className="text-white text-xs font-semibold text-center leading-relaxed max-w-[170px]">
...
function SongsVisual() {
  return (
    <div className="relative h-60 bg-gradient-to-br from-white/[0.04] to-black/20 flex flex-col justify-center gap-3 pl-4 pr-6 pt-4 pb-3">
      {/* Mock scrollbar */}
      <div className="absolute right-1.5 top-4 bottom-10 w-1 rounded-full bg-white/[0.06]">
        <div className="w-full h-[45%] rounded-full bg-white/20 mt-[10%]" />
      </div>
...
function PlaylistsVisual() {
  return (
    <div className="relative h-60 bg-[#0d2010] flex flex-col overflow-hidden">
      <div className="absolute inset-x-0 bottom-0 h-12 bg-gradient-to-b from-transparent to-black/60 pointer-events-none z-10" />

      {SPOTIFY_PLAYLISTS.map((pl, i) => (
        <div
          key={i}
          className="flex items-center gap-3 px-4 py-2.5 hover:bg-white/[0.04] transition-colors shrink-0"
        >
          {/* Thumbnail */}
          <div
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[9px] text-white/35">Made for you</p>
          </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {STEPS.map(({ number, Visual, title, description }) => (
            <div
              key={number}
              className="rounded-2xl overflow-hidden border border-white/10 bg-white/[0.03] hover:border-[#1DB954]/30 hover:bg-white/[0.05] transition-all duration-300 group"
            >
              {/* Visual mockup area */}
              <Visual />
        ...
        <p className="mt-10 text-center text-sm text-white/30">
          ...
          <span className="text-white/50">every song in your library</span> —
