...
const RECENT_SONGS = [
  ...
  { title: 'Blinding Lights', artist: 'The Weeknd', mood: 'Hype Mode' },
      ...
      <header className="sticky top-0 z-40 bg-[#F5F4F0]/80 backdrop-blur-md border-b border-black/[0.08]">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <a href="/" className="flex items-center gap-1">
            <span className="text-lg font-bold tracking-tight text-[#121212]">Mood</span>
            <span className="text-lg font-bold tracking-tight text-[#1DB954]">Sort</span>
          </a>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:flex items-center gap-2 bg-[#ECEAE4] border border-black/[0.08] rounded-full px-3 py-1.5">
              ...
              <span className="text-xs text-black/50 font-medium">Next sync in 6h</span>
            </div>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-sm font-medium text-[#121212] hidden sm:block">{displayName}</span>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 flex flex-col gap-12">
        ...
        <div>
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-lg font-bold text-[#121212]">Your mood playlists</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {PLACEHOLDER_PLAYLISTS.map((pl) => (
              ...
              >
                {/* Gradient cover art */}
                <div className={`h-28 bg-gradient-to-br ${pl.gradient} flex items-end p-4`}>
                  <span
                ...
                <div className="px-4 py-3.5">
                  <p className="text-sm font-bold text-[#121212] leading-tight">{pl.name}</p>
                  <p className="text-[11px] text-black/40 mt-0.5 leading-tight">{pl.description}</p>
                  <p className="text-[11px] text-black/30 mt-2">{pl.count} songs</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {RECENT_SONGS.map((song) => (
                <div key={song.title} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[12px] font-semibold text-[#121212] truncate leading-tight">{song.title}</p>
                    <p className="text-[11px] text-black/40 truncate leading-tight">{song.artist}</p>
                  </div>
                  <span className="text-[10px] text-black/30 shrink-0">{song.mood}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#ECEAE4]/60 border border-black/[0.08] rounded-2xl px-6 py-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-bold text-[#121212]">Pending review</h2>
              <span className="text-[10px] font-semibold bg-[#1DB954]/15 text-[#1DB954] rounded-full px-2 py-0.5">12 songs</span>
            </div>
            <p className="text-sm text-black/40 leading-relaxed mb-5">
              These songs couldn&apos;t be confidently clustered. Review them to keep your playlists tight.
            </p>
        ...
        <div className="bg-gradient-to-br from-[#1DB954]/8 to-[#1DB954]/3 border border-[#1DB954]/20 rounded-2xl px-6 py-6 flex flex-col sm:flex-row sm:items-center gap-5">
          ...
          <div className="flex-1">
            <p className="text-sm font-bold text-[#121212]">Full sync coming soon</p>
            <p className="text-sm text-black/45 mt-0.5">
              MoodSort will automatically sort all 1,284 of your liked songs once the AI pipeline is live.
            </p>
          </div>
